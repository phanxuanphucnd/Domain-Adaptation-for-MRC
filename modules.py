...
class Encoder(nn.Module):
    def __init__(
        ...
        elif configs.model_type in ['deberta', 'deberta-v3']:
            self.encoder = model.deberta
        elif configs.model_type == 'electra':
            self.encoder = model.electra
        else:
            raise ValueError(f"Model type `{configs.model_type}` not in list supported!")
...
class QAOutLayer(nn.Module):
    def __init__(self, model, freeze_qa_output_generator=False):
        super(QAOutLayer, self).__init__()
    ...
    def forward(self, encoder_out, start_positions=None, end_positions=None):
...
class DiscQAOutLayer(nn.Module):
    def __init__(
            self,
            freeze_aux_qa_output_generator=False,
            freeze_discriminator_encoder=False
    ):
        super(DiscQAOutLayer, self).__init__()
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=configs.emb_dim,
            dim_feedforward=3072,
            dropout=0.1,
            activation="gelu",
            nhead=12
        )
        self.discriminator_encoder = nn.TransformerEncoder(
            encoder_layer,
            num_layers=1
        )
