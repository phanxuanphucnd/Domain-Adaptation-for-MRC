...
from modules import Encoder, QAOutLayer, DiscQAOutLayer, ReverseLayerF


class AdaptMRCModel(nn.Module):
    def __init__(self):
        ...
        self.disc_aux_qa_layer = DiscQAOutLayer(
            freeze_aux_qa_output_generator=configs.freeze_aux_qa_output_generator,
    ...
    def forward(self, data_batch):
        ...
        factoid_qa_outputs, original_qa_loss = self.siamese_factoid_qa_out_gen(
            encodings,
            start_positions,
            end_positions
        )
        ...
        if configs.do_train:
            adv_loss, aux_qa_outputs, aux_qa_loss = self.siamese_discriminator(
                encodings,
                start_positions,
                end_positions
            )
    ...
    def gradient_reversal_layer(self, encoding):
        reversed_feature = ReverseLayerF.apply(
            encoding.last_hidden_state,
            configs.reverse_layer_lambda
        )  # ref: https://github.com/SongweiGe/scDGN/blob/master/utils/model_util.py
